...
async def get_code_from_account(account_id):
    ...
    client = TelegramClient(
        ...
        account.api_id or 6,
        account.api_hash or "eb06d4abfb49dc3eeb1aeb98ae0f581e"
    )
    try:
    ...
    async with async_session() as session:
        ...
        if purchase.code:
            ...
            await callback.message.edit_text(
                ...
                f'{"<tg-emoji emoji-id=\"" + em("lock_closed") + "\">🔒</tg-emoji> <b>2FA пароль:</b> <code>" + account.password_2fa + "</code>" if account.password_2fa else ""}\n',
                parse_mode=ParseMode.HTML
            )
            await callback.answer()
            return
        ...
        if code:
            ...
            await callback.message.edit_text(
                ...
                f'{flag} <b>Страна:</b> {country.name if country else "Неизвестно"}\n'
                f'<tg-emoji emoji-id="{em("file")}">📁</tg-emoji> <b>Номер:</b> <code>{account.phone}</code>\n'
                f'<tg-emoji emoji-id="{em("code")}">🔨</tg-emoji> <b>Код:</b> <code>{code}</code>\n'
                f'{"<tg-emoji emoji-id=\"" + em("lock_closed") + "\">🔒</tg-emoji> <b>2FA пароль:</b> <code>" + account.password_2fa + "</code>" if account.password_2fa else ""}\n',
                parse_mode=ParseMode.HTML
            )
        else:
...
async def add_account_api_id(message: Message, state: FSMContext):
    ...
    try:
        api_id = int(message.text.strip()) if message.text.strip() else 6
    except ValueError:
        api_id = 6
    ...
    await message.answer(
        ...
        f'<i>По умолчанию: eb06d4abfb49dc3eeb1aeb98ae0f581e</i>',
        parse_mode=ParseMode.HTML,
        reply_markup=get_admin_cancel_keyboard()
    )
...
async def add_account_api_hash(message: Message, state: FSMContext):
    ...
    api_hash = message.text.strip() or "eb06d4abfb49dc3eeb1aeb98ae0f581e"
...
async def add_account_2fa(message: Message, state: FSMContext):
    ...
    phone = data["phone"]
    api_id = data.get("api_id", 6)
    api_hash = data.get("api_hash", "eb06d4abfb49dc3eeb1aeb98ae0f581e")
